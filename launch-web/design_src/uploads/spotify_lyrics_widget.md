...
- 尚未做 code signing / antivirus false-positive 處理
...
## V3 portable onboarding（已 merge 到 master）

Branch：`codex/v3-portable-onboarding`，已 merge 到 `master` through `189e99e`。

實作內容：

- `src/onboarding.py`：first-run 設定 dialog（開啟 Dashboard、複製 Redirect URI、輸入 Client ID）
- `src/main.py`：`App.start()` 缺少 `client_id` 時彈 onboarding dialog
- `README.md`：中文使用者教學（portable zip 使用方式、Spotify 設定、資料位置）
- `SpotifyLyricsWidget.spec`：PyInstaller one-folder build（`console=False`、`upx=False`、bundled font）
- `scripts/build_portable.ps1`：build + 複製 README + 壓 zip，有防呆檢查
- `.gitignore`：允許 commit `SpotifyLyricsWidget.spec`
- `requirements.txt`：加入 `pyinstaller>=6.0.0`

測試：

- `tests/test_onboarding.py`（5 tests）
- `tests/test_main.py`（新增 3 個 onboarding 整合 tests）
- `tests/test_packaging.py`（6 tests）
- `tests/test_readme.py`（1 test）

驗證（2026-06-08）：

- V3 focused tests：`16 passed`
- Full suite：`224 passed`（16 errors 為既有 temp 目錄權限問題，master 上也相同，非 V3 引入）
- Portable build：成功產出 `dist/SpotifyLyricsWidget-v3-portable/` 和 `.zip`（~80 MB）
...
## V3.1 Windows installer + taskbar controller（branch `codex/installer`）

Branch：`codex/installer`，worktree `.worktrees/installer`。

實作內容：

- `src/taskbar_host.py` / `src/main.py`：新增 taskbar controller lifecycle。app 啟動後保留 taskbar 入口，controller 可啟動、顯示、隱藏、關閉 widget；重複開啟仍聚焦既有 instance。
- `src/windows_app_id.py`：設定穩定 Windows AppUserModelID。
- `installer/SpotifyLyricsWidget.iss`：Inno Setup installer，版本 metadata `3.1.0`，安裝到 `{localappdata}\Programs\Spotify Lyrics Widget`。
- `scripts/build_installer.ps1`：只負責把既有 PyInstaller one-folder output 編成 installer，不重新打包 Python app。
- `tests/test_installer.py`：覆蓋 installer metadata、安裝來源、shortcut icon、postinstall launch、shell icon refresh、禁止 startup/taskbar pin。

驗證：

- Installer focused tests：`9 passed`
- Full suite：`282 passed`
- Inno Setup build：成功產出 `dist/SpotifyLyricsWidgetSetup.exe`

文件：

- Spec：`docs/superpowers/specs/2026-06-21-spotify-widget-taskbar-host-installer-design.md`
- Taskbar plan：`docs/superpowers/plans/2026-06-22-spotify-widget-taskbar-host.md`
- Installer plan：`docs/superpowers/plans/2026-06-24-spotify-widget-installer.md`
