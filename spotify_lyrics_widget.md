...
目前實作版本：V3 portable onboarding（branch `codex/v3-portable-onboarding`，待 merge 到 `master`）

下一步：merge V3 到 master、manual smoke test、考慮 code signing / antivirus false-positive 處理。
...
## V3 portable onboarding（branch `codex/v3-portable-onboarding`）

Branch：`codex/v3-portable-onboarding`，worktree `.worktrees/v3-portable-onboarding`。
...
1. Merge V3 到 master，manual smoke test（乾淨環境跑 exe 走完 onboarding）。
2. 處理 antivirus false-positive / code signing。
3. LICENSE 檔案。
4. Playlist add / default playlist（deferred）。
